...
public class CheckInOutController {
    ...
    @FXML private void handleSearch() {
        if (txtDeviceSearch.getText().trim().isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Please enter an Access Code, Student ID, or Serial Number."); return;
        }
    ...
    @FXML private void handleConfirm() {
        if (lblStudentID.getText().isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Please search for a device/student first."); return;
        }
    ...
    @FXML private void handleClear() {
        txtDeviceSearch.clear(); txtRemarks.clear();
        lblStudentName.setText(""); lblStudentID.setText("");
        lblDevice.setText(""); lblSerialNumber.setText("");
        lblAccessCode.setText(""); lblCurrentStatus.setText("");
        cmbAction.getSelectionModel().selectFirst();
        cmbLocation.getSelectionModel().selectFirst();
        cmbGuard.getSelectionModel().selectFirst();
        updateDateTime();
